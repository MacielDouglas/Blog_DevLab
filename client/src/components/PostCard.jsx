  ...
  return (
    <div className="group relative w-[380px] border border-transparent hover:border-base_01  hover:bg-stone-100 h-[350px] overflow-hidden transition-all m-3 shadow-md">
      ...
      <div className="py-[0.1rem] px-3 flex flex-col gap-2">
        ...
        <p className="text-lg font-semibold line-clamp-2 group-hover:pt-2">
          {post.title}
        </p>
        <span className="italic text-sm">{post.category}</span>
        <Link
          to={`/post/${post.slug}`}
          className="z-10 group-hover:bottom-0 absolute bottom-[-200px] left-0 right-0 border bg-base_03 text-base_01 transition-all duration-300 text-center py-2 rounded-md !rounded-tl-none m-2"
        >
          Leia o artigo
        </Link>
      </div>
